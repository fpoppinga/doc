...
const messagingService = new WsMessagingService(parseInt(process.env["WEBSOCKET_PORT"] || "8081", 10));
...
app.get("/events/:since", async (req, res) => {
    return res.status(200).send(await eventStore.getSince(parseInt(req.params.since, 10)));
});
